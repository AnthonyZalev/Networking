...
import pa1.autograding.GradingState;
...
public class FileServer extends AbstractNIOServer {
	...
	private static final long MAINTENANCE_INTERVAL = 180000; // ms
	...
	protected void executeRequest(String request, SocketChannel connChannel) {
		...
		try {
			...
			log.info("Received " + (FileManager.getChunkNumber(request) == -1
					? GradingState.Keys.RECEIVED_FULL_REQUEST : GradingState
					.Keys.RECEIVED_CHUNK_REQUEST) + " request on " +
					connChannel + ": " + request);
	...
	private boolean addServer(int port) {
		if (port == Config.DEFAULT_FIXED_PORT) return false;
		boolean added = false;
		if (this.canAddServerPort(port)) {
			ServerSocketChannel ssc = this.openServer(port);
			added = (ssc != null) && ssc.isOpen() && this.portToServers
					.containsKey(port);
			if (added) sharedInfo.addPort(port);
		}
		return added;
	}
