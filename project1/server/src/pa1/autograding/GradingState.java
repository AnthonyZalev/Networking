...
public class GradingState {
	...
	public static enum Keys {
		...
		LAST_INIT_TIME, // the time when the earliest grading server testwas initiated <br>
	...
	// unused
	private final int DEFAULT_TESTING_ID = 12345;
	...
	private static class FileStats {
		...
		FileStats(String filename) {
			...
			this.lastInitTime = this.initTime;
		}
		...
		FileStats(JSONObject json) throws JSONException {
			...
			this.lastInitTime = json.getLong(Keys.LAST_INIT_TIME.toString()); // must
			// exist
		...
		public JSONObject toJSON() throws JSONException {
			...
			json.put(Keys.LAST_INIT_TIME.toString(), this.lastInitTime);
		...
		public void merge(FileStats fstats) {
			...
			this.lastInitTime = fstats.lastInitTime;
			...
			this.receivedChunkRequest = this.receivedChunkRequest || fstats
					.receivedChunkRequest;
			this.receivedChallengeResponse =
					this.receivedChallengeResponse ||
		...
		public boolean setReceivedChunkRequest() {
			boolean old = this.receivedChunkRequest;
			this.receivedChunkRequest = true;
			return !old;
		}
	}
	...
	public synchronized void processRequest(String request, int port) {
		...
		if (FileManager.isLegitTorrentRequest(request)) {
			...
			if(request.contains(Config.DEFAULT_SEPARATOR_STRING))
				this.setChanged(this.curFstats.setReceivedChunkRequest());
